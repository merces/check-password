 ...
 *
 * Copyright (c) 2013, 2015, 2025-2026 Fernando Mercês
 *
...
void loadconfig() {
  ...
  while (fgets(line, MAX_LINE, fp)) {
    ...
    if (param && (strcmp("log", param) == 0)) {
      ...
      if (value)
        strncpy(config.logfile, value, MAX_LOGFILE - 1);
    }
  }
  fclose(fp);
}
...
static int checkdate(const char *pw) {
  ...
  if (strstr(pw, year)) {
    if (config.log) {
      snprintf(msg, MAX_LOG_MSG, "password contains the current year (%s)",
               year);
      logit(msg);
    }
    return 0;
  ...
  if (strcasestr(pw, mname)) {
    if (config.log) {
      snprintf(msg, MAX_LOG_MSG,
               "password contains the current month name (%s)", mname);
      logit(msg);
    }
    return 0;
  }
  if (strstr(pw, mnum)) {
    if (config.log) {
      snprintf(msg, MAX_LOG_MSG,
               "password contains the current month number (%s)", mnum);
      logit(msg);
    }
    return 0;
  }
...
int check_password(const char *pPasswd, char **ppErrStr __attribute__((unused)),
                   const Entry *pEntry) {
  loadconfig();

  if (!basic_rules(pPasswd) || !keyboard_seq(pPasswd) || !checkdate(pPasswd) ||
      !checkusername(pEntry->e_name.bv_val, pPasswd))
    return 1;
